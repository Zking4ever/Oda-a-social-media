...
if(count($response)>0){
    ...
    if(count($filtered)>0){
        ...
        //making post visible which are not currently visible
        if(isset($_GET['visiblePosts'])){
            
            $visiblePosts = array();
            $invisiblePosts = array();

            $visiblePosts = json_decode($_GET['visiblePosts']);
            $invisiblePosts = subtractArray($fetchedPost,$visiblePosts); // my own function
            
            if(count($invisiblePosts)>0){
